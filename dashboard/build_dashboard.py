...
<section id="supplier-view" class="tab-panel" role="region" aria-label="Supplier detail">
  <p><a class="back" href="#/suppliers">← Back to suppliers</a></p>
  <p class="hint" id="supplier-crumbs"></p>
...
(function () {{
  ...
  nav.addEventListener('click', function (e) {{
    ...
    if (location.hash.indexOf('#/stage/') === 0 ||
        location.hash.indexOf('#/supplier/') === 0) {{
      // leaving a detail page via the tab bar: normalize the URL without a
  ...
  function fillSupplier(name) {{
    ...
    document.getElementById('supplier-crumbs').textContent =
      'Home › Suppliers › ' + page.label;
  ...
  function route() {{
    ...
    // the Suppliers tab's own address (Back button from a supplier card)
